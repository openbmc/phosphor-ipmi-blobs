...
void loadLibraries(const std::string& path)
{
    ...
    for (const auto& p : fs::recursive_directory_iterator(path))
    {
        ...
        if (libHandle)
        {
            ...
            factory = (std::unique_ptr<GenericBlobInterface> (*)(void))dlsym(libHandle, "createHandler");

            const char *error = dlerror();
            if (error)
            {
                log<level::ERR>("ERROR loading symbol", entry("HANDLER=%s", ps.c_str()), entry("ERROR=%s", error));
            }
            else
            {
                ...
                if (!result)
                {
                    log<level::ERR>("Unable to create handler", entry("HANDLER=%s", ps.c_str()));
                    continue;
                }
