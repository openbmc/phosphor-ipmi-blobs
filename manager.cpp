...
void BlobManager::cleanUpStaleSessions(GenericBlobInterface* handler)
{
    ...
    std::copy_if(
        openSessions[handler].begin(), openSessions[handler].end(),
        std::inserter(expiredSet, expiredSet.begin()), [](auto sessionId) {
            now_t - sessions[sessionId].lastActionTime >= sessionTimeout
        });
