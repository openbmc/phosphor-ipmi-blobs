                                       ...
                                       uint32_t requestedSize)
{
    ...
    /* TODO: Currently, configure_ac isn't finding libuserlayer, w.r.t the symbols I need.
     */
    ...
    /** The requested bytes will have a CRC. */
    const int crcSize = sizeof(uint16_t);

    uint32_t maxTransportSize = 64; // ipmi::getChannelMaxTransferSize(ipmiChannel);
    maxTransportSize -= ipmiBtReplyHdrLen;
    maxTransportSize -= oem::groupMagicSize;
    maxTransportSize -= crcSize;

    /* Check if the requestedSize is more than a reply can be. */
    std::fprintf(stderr, "maxtransport size remaining: %d\n", maxTransportSize);

    /* Try reading from it. */
    return info->handler->read(session, offset, std::min(maxTransportSize, requestedSize));
}
